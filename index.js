...
<a/>
    ...
    `,i=function(t,s){return t.classList.toggle(s)},h=function(){return new m(".gallery a")},u=document.querySelector(".form"),c=document.querySelector(".gallery"),l=document.querySelector(".js-loader"),g=t=>{t.preventDefault(),i(l,"visually-hidden"),c.innerHTML="";const s=t.currentTarget.elements.input.value.trim();if(s===""){d.error({title:"Error",message:"Please enter your search query"}),i(l,"visually-hidden");return}y(s).then(o=>{if(o.total===0){d.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!"}),c.innerHTML="",i(l,"visually-hidden"),u.reset();return}const a=o.hits.map(e=>f(e)).join("");i(l,"visually-hidden"),c.innerHTML=a,h(),u.reset()}).catch(o=>console.log(o))};u.addEventListener("submit",g);
